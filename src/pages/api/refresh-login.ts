...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    const { password, newPassword, action } =
      (await request.json()) as NotionPassword;
    ...
    if (passwords.length === 0 || !passwords[0].properties.Password.title) {
      return new Response(
        JSON.stringify({ error: "Password not found in Notion database" }),
        { status: 500 }
      );
    }
    ...
    if (!notionPassword) {
      return new Response(
        JSON.stringify({ error: "No password stored in Notion database" }),
        { status: 500 }
      );
    }
  ...
  async function handleRefresh(
    ...
    if (password === notionPassword) {
      try {
        ...
        return new Response(
          JSON.stringify({ error: "Failed to refresh data" }),
          { status: 500 }
        );
      }
    } else {
